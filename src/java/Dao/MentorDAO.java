...
public class MentorDAO extends DBConnect {
    ...
    public List<User> getAllMentor() {
        ...
        try (PreparedStatement stm = connection.prepareStatement(query); ResultSet rs = stm.executeQuery()) {
            while (rs.next()) {
                ...
                mentor.setUser_id(rs.getInt("user_id"));
                mentor.setAccount_id(rs.getInt("account_id"));
    ...
    public List<User> getMenteesById(int mentorId) {
        List<User> list = new ArrayList<>();
        String query = """
                        SELECT distinct u.user_id, u.name, u.gender, u.DOB, u.phone, 
                        u.address, u.email, u.avatar, u.account_id, u.status
                        FROM Request r
